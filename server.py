...
while(1):
    ...
    con.send(b"Hi client, what do u want me to send! press 1 for timestamp of file or press 2 for transfer file")
    d=con.recv(1024)
    print(d)
    ...
    if d==b'1':
        ...
        timestamp=0
        with open(file) as f:
            timestamp = os.stat(file).st_mtime #to fetch timestamp
            con.send(bytes(time.ctime(timestamp),'utf-8')) #sending timestamp
        f.close()        
